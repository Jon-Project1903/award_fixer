...
export const api = {
  ...
  toggleErroneous: (id: number) =>
    fetch(`${BASE}/reconciliations/${id}/erroneous`, { method: 'PUT' }).then(r => json<any>(r)),

  resolveAllPassed: (projectId: number) =>
    fetch(`${BASE}/projects/${projectId}/resolve-all-passed`, { method: 'POST' }).then(r => json<any>(r)),
  ...
  updateInventorCity: (projectId: number, inventorKey: string, city: string, state?: string) =>
    fetch(`${BASE}/projects/${projectId}/inventors/city`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ inventor_key: inventorKey, city, state }),
    }).then(r => json<any>(r)),
